...
namespace DoAn_LapTrinhWeb.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //hiển thị thịt cá hải sản được mua nhiều nhất

            //List<Product> kitchensales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 4).Take(4).OrderByDescending(item => item.buyturn).ToList();
            //ViewBag.KitchenSales = kitchensales;
            List<Product> kitchensales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 20).Take(4).OrderByDescending(item => item.buyturn).ToList();
            ViewBag.KitchenSales = kitchensales;
            //hiển thị những màn hình được mua nhiều nhất
            List<Product> fishsales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 21).Take(4).OrderByDescending(item => item.buyturn).ToList();
            ViewBag.Fishsales = fishsales;

            //hiển thị những laptop được mua nhiều nhất
            List<Product> laptopsales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 22).Take(4).OrderByDescending(item => item.buyturn).ToList();
            ViewBag.LaptopSales = laptopsales;
            //hiển thị những linh kiện điện tử được mua nhiều nhất
            List<Product> tablesales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 23).Take(4).OrderByDescending(item => item.buyturn).ToList();
