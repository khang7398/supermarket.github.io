...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class ProductsAdminController : BaseController
    {
        ...
        //View list sản phẩm
        public ActionResult
            ProductIndex(int? size, int? page, string productName, string show,
                string sortOrder) // hiển thị tất cả sp online
        {
        ...
        public ActionResult
            ProductTrash(string product_name, string show, int? size, int? page,
                string sortOrder) // hiển thị tất cả sp online
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
        ...
        public ActionResult ProductCreate()
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                ...
                ViewBag.ListDiscount =
                    new SelectList(
                        _db.Discounts.Where(m => m.status == "1" && m.discounts_type == 1)
                            .OrderByDescending(m => m.disscount_id), "disscount_id", "discount_name", 0);
                ViewBag.ListBrand =
                    new SelectList(
                        _db.Brands.Where(m => (m.status == "1" || m.status == "0")).OrderBy(m => m.brand_name),
                        "brand_id", "brand_name", 0);
                ViewBag.ListGenre =
                    new SelectList(
                        _db.Genres.Where(m => (m.status == "1" || m.status == "0")).OrderBy(m => m.genre_name),
                        "genre_id", "genre_name", 0);
                return View();
            }
            else
            {
        ...
        public async Task<ActionResult> ProductCreate(ProductDTOs Prodtos, Product product, Product_Image product_image)
        {
            ...
            ViewBag.ListParentGenre = new SelectList(_db.ParentGenres.Where(m => m.status == "1").OrderBy(m => m.name),
                "id", "name", 0);
            ViewBag.ListDiscount =
                new SelectList(
                    _db.Discounts.Where(m => m.status == "1" && m.discounts_type == 1)
                        .OrderByDescending(m => m.disscount_id), "disscount_id", "discount_name", 0);
            ViewBag.ListBrand =
                new SelectList(_db.Brands.Where(m => (m.status == "1" || m.status == "0")).OrderBy(m => m.brand_name),
                    "brand_id", "brand_name", 0);
            ViewBag.ListGenre =
                new SelectList(_db.Genres.Where(m => (m.status == "1" || m.status == "0")).OrderBy(m => m.genre_name),
                    "genre_id", "genre_name", 0);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (checkslug)
                    {
                        product.slug = slug + "-" + DateTime.Now.ToString("HH") + DateTime.Now.ToString("mm") +
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult ProductEdit(Product productDtOs, string returnUrl, int id)
        {
            ViewBag.ListDiscount =
                new SelectList(
                    _db.Discounts.Where(m => m.status == "1" && m.discounts_type == 1)
            ...
            var discount = _db.Discounts.FirstOrDefault(m => m.disscount_id == productDtOs.disscount_id);
            try
            {
        ...
        {
            Product product = _db.Products.Where(m => m.product_id == id).FirstOrDefault();
            int title = product.product_id;
            product.status = state.ToString();
            product.update_at = DateTime.Now;
            product.updateby = User.Identity.GetEmail();
            _db.SaveChanges();
            return Json(new { Message = "Đã chuyển trạng thái ID" + title }, JsonRequestBehavior.AllowGet);
        }
