...
var Edit = function (id, username, phonenumber, address_content) {
    $('#nameorderaddress').val(username);
    $('#phoneorderaddress').val(phonenumber);
    $('#addresscontent').val(address_content);
    editModal2.modal('show');
    ide = id 
    $('#btnaddressedit').click(function () {
    var _username = $("#nameorderaddress").val();
    var _phonenumber = $("#phoneorderaddress").val();
    var _address = $("#addresscontent").val();
        if (_username == "" || _phonenumber == "" || _address == "") {
            const Toast = Swal.mixin({
                toast: true,
                position: 'top',
                showConfirmButton: false,
            ...
            Toast.fire({
                icon: 'warning',
                title: 'Địa chỉ không quá 150 ký tự'
            })
        }
        else{
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ id: ide, username: _username, address_content: _address, phonenumber: _phonenumber}),
            dataType: "json",
            success: function (result) {
                editModal2.modal('hide');
                if (result == true) {
                    ...
                    $(".update_address").text("Địa chỉ: " + _address);
